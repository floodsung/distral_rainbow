...
class BatchedPlayer(Player):
    ...
    def _step_sub_batch(self, sub_batch):
        ...
        for i, (obs, rew, done, info) in enumerate(zip(*outs)):
            ...
            if done:
                self.recent_rewards.append(self._total_rewards[sub_batch][i])
                if len(self.recent_rewards) > 10:
                    self.recent_rewards.popleft()
                self.episodes_counter += 1
                print("episode:",self._episode_ids[sub_batch][i],"current reward:",self._total_rewards[sub_batch][i],"average reward:",np.mean(list(self.recent_rewards)))
